...
| B1-14 | Canvas 규격 통일 (Scale With Screen Size) | `development-plan` 6절 #4 | ⛔ `15-screen.md` 확정 대기 (제안 작성됨, 2-1 순서 2) |
...
| 순서 | 작업 | 선행 조건 | 규모 | 내용 |
| --- | --- | --- | --- | --- |
| 1 | **화면 규격 확정** | 사용자 결정 (`15-screen.md` 3절 #1 · #3) | 결정만 | 가로 고정 · Expand. 두 가지만 정하면 나머지는 따라온다 |
...
| 3 | **방 화면 재구성 (뷰)** | `08-room` 5절 제안 확정 | 1d | 로비와 같은 방식 — 배치 규칙 문서(`16-room-ui.md`) 신설 → 디자인 없이 기본 스프라이트로 씬 재구성. 요소: 방 코드(크게, 복사 가능) · 플레이어 목록 4칸(닉네임 · 방장 표시) · 인원 · 시작(방장) · 나가기 · 로그. U-24 `RoomPlayerListView` 도 여기서 |
...
> 1 · 3 의 결정은 `15-screen.md` 3절과 `08-room.md` 5절에 **제안 값을 적어 두었다.** 그대로 확정하면 2 · 3 을 바로 시작한다.
