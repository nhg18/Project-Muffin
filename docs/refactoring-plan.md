...
> 2부 Phase 2~7 의 **순서 · 일정**, 3부 우선순위, 4부 기획 확정 목록은 [`development-plan.md`](development-plan.md) v4 (M1~M8 · 4-1 마감표)와 [`plan-a-logic.md`](plan-a-logic.md) · [`plan-b-ui.md`](plan-b-ui.md) 로 대체됐다. 2부의 **설계 내용**(GameServer · CardInstance · EffectContext …)은 설계 참고로 남긴다.
> 5부 금지 항목 중 "새 카드 에셋" · "함정 슬롯 UI" 두 줄은 샌드박스([`systems/18-sandbox.md`](systems/18-sandbox.md))로 풀렸다 — 5부 주석 참고.
...
* ~~Phase 2 이전에 **새 카드 에셋을 추가하지 않는다.** 전부 다시 만들게 된다.~~
  → 2026-09-26 해제: 카드 데이터는 효과 없이 표 가져오기 도구로 만든다(B1-4). 숫자가 확정되면 같은 도구로 다시 가져온다.
* ~~Phase 2 이전에 **함정 슬롯 UI를 만들지 않는다.** 데이터 구조가 없다.~~
  → 2026-09-26 해제: 함정 칸은 서버 1층의 영역으로 먼저 생긴다(A1-3). UI 는 그 영역을 표시한다(B1-2).
